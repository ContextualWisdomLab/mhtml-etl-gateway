        ...
        def adapted_rows() -> Iterable[tuple[Any, ...]]:
            ...
            for offset, row in enumerate(rows):
                ...
                if row_len >= num_cols:
                    ...
                    values.extend([None] * (num_cols - row_len))
...
def prepare_typed_rows(
    ...
    for row in rows:
        row_len = len(row)
        if row_len >= num_cols:
            app([coerce_value(str(row[i]), pg_types[i]) for i in range(num_cols)])
        else:
            app(
                [
                    (
                        coerce_value(str(row[i]), pg_types[i])
                        if i < row_len
                        else coerce_value("", pg_types[i])
                    )
                    for i in range(num_cols)
                ]
            )
