            ...
            for offset, row in enumerate(rows):
                ...
                if row_len >= num_cols:
                    ...
                    for i in range(min(num_cols, row_len)):
                        val = row[i]
                        values[i] = (
                            coerce_value(val, col_types[i]) if type(val) is str else val
                        )

                values.append(source_artifact_path)
                values.append(source_artifact_sha256)
                values.append(start_row_number + offset)
                yield tuple(values)
    ...
    return [
        (
            ...
            if len(row) >= num_cols
                ...
                coerce_value(str(row[i]) if i < len(row) else "", col_types[i])
                for i in range(num_cols)
            ]
        )
        for row in rows
