...
def _build_row_records(
    ...
    *,
    ...
    cols = [(i, col.db_name) for i, col in enumerate(schema.columns)]
    records: list[dict[str, Any]] = []
    app = records.append
    for offset, row in enumerate(rows):
        row_len = len(row)
        record: dict[str, Any] = {
            db_name: (row[i] if i < row_len else None) for i, db_name in cols
        }
    ...
    def write_artifact_rows(
        ...
        *,
        ...
        def adapted_rows() -> Iterable[tuple[Any, ...]]:
            cols = [(i, col.pg_type) for i, col in enumerate(schema.columns)]
            for offset, row in enumerate(rows):
                row_len = len(row)
                yield tuple(
                    [
...
def prepare_typed_rows(
    ...
    for row in rows:
        ...
        app(
            [
                coerce_value(str(row[i]) if i < row_len else "", pg_type)
                for i, pg_type in cols
            ]
        )
    return prepared
