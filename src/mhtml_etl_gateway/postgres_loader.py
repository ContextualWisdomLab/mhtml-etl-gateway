...
def _build_row_records(
    ...
    *,
    ...
    num_cols = range(len(db_names))
    records: list[dict[str, Any]] = []

    # ⚡ Bolt: Cache method lookup to avoid overhead in tight loop
    app = records.append
    for offset, row in enumerate(rows):
        row_len = len(row)
        record: dict[str, Any] = {}
        for i in num_cols:
            record[db_names[i]] = row[i] if i < row_len else None
