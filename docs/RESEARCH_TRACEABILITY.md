...
| DBML visualization handoff | DBML. (n.d.). *DBML syntax—Core database markup*. Retrieved August 12, 2026, from https://dbml.dbdiagram.io/docs/; ContextualWisdomLab. (2026). *pg-erd-cloud DBML conversion API* [Source code, commit 72afe6db712b145baaba084f64a1ff4fb36d9fd0]. GitHub. https://github.com/ContextualWisdomLab/pg-erd-cloud/blob/72afe6db712b145baaba084f64a1ff4fb36d9fd0/backend/app/api/dbml.py | emit a transport-neutral `POST /api/dbml/convert` plan with one value-free table, allow-listed types, and explicit nullability; keep authentication and persistence caller-owned |
| value-free proposal JSON interchange | Bray, T. (Ed.). (2017). *The JavaScript Object Notation (JSON) Data Interchange Format* (RFC 8259). Internet Engineering Task Force. https://doi.org/10.17487/RFC8259 | serialize the local proposal as one portable JSON object while retaining the value-free privacy boundary |
