...
## Current 0.4.0 implementation addendum
...
value-free schema proposals, `COMMENT ON COLUMN` mapping support, the
Semantic Data Portal connector boundary, and a bounded local `propose`
CLI/Python wrapper from validated MHTML tables. The worktree on 2026-08-12 is
validated with Python 3.14 as follows:
...
The release candidate reports package version `0.4.0`; `uv build --wheel`
produced `mhtml_etl_gateway-0.4.0-py3-none-any.whl`. Build artifacts remain
...
The `mhtml_etl_gateway-0.4.0-py3-none-any.whl` remains outside the repository
and is subject to the exact-head release checks.
