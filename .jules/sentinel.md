...
## 2025-02-18 - [Fix Hardcoded SQL Expressions]
**Vulnerability:** Possible SQL injection vector through string-based query construction using f-strings with `IN ({placeholders})`.
**Learning:** Avoid using string interpolation (f-strings) to construct `IN ({placeholders})` clauses for SQL queries, as it triggers static security scanner (Bandit B608) warnings.
