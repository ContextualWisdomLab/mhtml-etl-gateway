...
def test_loader_adapted_rows_coverage(monkeypatch) -> None:
    ...
    monkeypatch.setattr(psycopg, "connect", lambda *a, **k: MockConn())
    sink = PsycopgSink("dummy")
    schema = TableSchema(
        table_name="safe_table_name",
        columns=[
            ColumnSpec("safe_col_a", "safe_col_a", PG_TEXT),
            ColumnSpec("safe_col_b", "safe_col_b", PG_TEXT),
        ],
    )
    rows = [["only_one_col"]]
    entry = make_catalog_entry(
